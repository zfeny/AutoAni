...
class MikanPageScraper:
    ...
    def scrape_episode_page(self, episode_url: str) -> Optional[Dict]:
        ...
        try:
            ...
            print(f"Failed to scrape episode page {episode_url}: {e}")
            return None
    ...
    def _extract_raw_rss_url(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        try:
            ...
            # 转换为完整 URL
            raw_rss_url = urljoin(self.BASE_URL, href)
            return raw_rss_url

        except Exception as e:
            print(f"Failed to extract raw_rss_url: {e}")
            return None
    ...
    def _extract_img_url(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        try:
            ...
            # 去除查询参数（width, height, format）
            img_path = img_path.split('?')[0]

            # 转换为完整 URL
            img_url = urljoin(self.BASE_URL, img_path)
            return img_url

        except Exception as e:
            print(f"Failed to extract img_url: {e}")
            return None
    ...
    def scrape_bangumi_page_from_rss_url(self, raw_rss_url: str) -> Optional[Dict]:
        ...
        try:
            ...
            print(f"Failed to scrape bangumi page from {raw_rss_url}: {e}")
            return None
    ...
    def _fetch_series_name_from_rss(self, rss_url: str) -> Optional[str]:
        ...
        try:
            ...
            # 从 channel title 提取
            # 格式: "Mikan Project - 永远的黄昏"
            channel_title = feed.feed.get('title', '')
            if channel_title:
                # 去除 "Mikan Project - " 前缀
                series_name = channel_title.replace('Mikan Project - ', '').strip()
                return series_name

            return None

        except Exception as e:
            print(f"Failed to fetch series name from RSS {rss_url}: {e}")
            return None
